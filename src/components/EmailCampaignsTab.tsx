...
type EmailCampaign = {
  id: string; nome: string; assunto: string; mensagem: string | null;
  status: string; total_contatos: number; enviados: number; falhas: number;
...
type ICP = { id: string; nome: string; leadCount?: number; emailCount?: number };

function downloadEmailCsvTemplate() {
  const content = 'email;nome\njoao@empresa.com;João Silva\nmaria@empresa.com;Maria Santos';
  const blob = new Blob([content], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = 'modelo_contatos_email.csv'; a.click();
...
export default function EmailCampaignsTab() {
  ...
  const fetchCampaigns = useCallback(async () => {
    if (!tenantId) return;
    const { data } = await supabase.from('email_campaigns').select('*').eq('tenant_id', tenantId).order('created_at', { ascending: false });
  ...
  const fetchIcps = useCallback(async () => {
    ...
    if (icpData) {
      ...
      for (const icp of icpData) {
        const { data: runs } = await supabase.from('runs').select('id').eq('icp_id', icp.id).eq('tenant_id', tenantId);
        const runIds = runs?.map(r => r.id) || [];
        if (runIds.length === 0) { icpsWithCounts.push({ ...icp, leadCount: 0, emailCount: 0 }); continue; }
        const { data: leads } = await supabase.from('leads').select('raw_json').eq('tenant_id', tenantId).in('run_id', runIds);
        const emailCount = (leads || []).filter(l => {
          const raw = (l as any).raw_json || {};
          const emails = raw.emails || raw.email;
          if (Array.isArray(emails)) return emails.length > 0;
          return typeof emails === 'string' && emails.includes('@');
        }).length;
        icpsWithCounts.push({ ...icp, leadCount: leads?.length || 0, emailCount });
      }
  ...
  const handleCsvUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (emailIdx === -1) { toast.error('Coluna "email" não encontrada'); return; }
  ...
  const handleCreate = async () => {
    ...
    if (!mensagem.trim()) { toast.error('Mensagem é obrigatória'); return; }
    setCreating(true);
    try {
      let contactsToInsert: { email: string; nome: string | null; lead_id: string | null }[] = [];
      ...
      if (contactSource === 'csv') {
        ...
        // Auto-add email typed in the field but not yet added via "+"
        let finalManualContacts = [...manualContacts];
        const pendingEmail = manualEmail.trim();
        if (pendingEmail.includes('@') && !finalManualContacts.some(c => c.email === pendingEmail)) {
          finalManualContacts.push({ email: pendingEmail, nome: manualNome.trim() });
        }
        contactsToInsert = finalManualContacts.map((c) => ({ email: c.email, nome: c.nome || null, lead_id: null }));
      } else {
        ...
        const { data: leadsData } = await supabase.from('leads').select('id, razao_social, raw_json').eq('tenant_id', tenantId!).in('run_id', runIds);
        contactsToInsert = (leadsData || []).map((l) => {
          const raw = (l as any).raw_json || {};
          const emails = raw.emails || raw.email;
          let email = '';
          if (Array.isArray(emails) && emails.length > 0) email = typeof emails[0] === 'string' ? emails[0] : (emails[0]?.email || '');
          else if (typeof emails === 'string') email = emails;
          return { email, nome: l.razao_social, lead_id: l.id };
        }).filter((c) => c.email.includes('@'));
      }

      if (contactsToInsert.length === 0) { toast.error('Nenhum contato com e-mail válido'); setCreating(false); return; }

      const { data: campaign, error: campErr } = await supabase.from('email_campaigns')
        .insert({ tenant_id: tenantId!, nome, assunto, mensagem, total_contatos: contactsToInsert.length, use_ai_variations: useAiVariations } as any)
        .select().single();
      if (campErr) throw campErr;

      const { error: contactErr } = await supabase.from('email_campaign_contacts').insert(contactsToInsert.map((c) => ({ campaign_id: campaign.id, ...c })) as any);
  ...
  const openEdit = (c: EmailCampaign) => {
    setEditingCampaign(c); setEditNome(c.nome); setEditAssunto(c.assunto); setEditMensagem(c.mensagem || '');
  ...
  const handleEditSave = async () => {
    ...
    try {
      const { error } = await supabase.from('email_campaigns')
        .update({ nome: editNome.trim(), assunto: editAssunto.trim(), mensagem: editMensagem.trim() || null } as any)
        .eq('id', editingCampaign.id);
      if (error) throw error;
      toast.success('Campanha atualizada'); setShowEditDialog(false); fetchCampaigns();
  ...
  const handleDelete = async (campaignId: string) => {
    setDeleting(true);
    try {
      await supabase.from('email_campaign_contacts').delete().eq('campaign_id', campaignId);
      const { error } = await supabase.from('email_campaigns').delete().eq('id', campaignId);
      if (error) throw error;
      toast.success('Campanha excluída'); setDeleteConfirm(null); fetchCampaigns();
  ...
  const handleSend = async (campaignId: string) => {
    ...
    try {
      ...
      if (!session) return;
      const projectId = import.meta.env.VITE_SUPABASE_PROJECT_ID;
      const res = await fetch(`https://${projectId}.supabase.co/functions/v1/send-email`, {
  ...
  const handleViewDetails = async (campaignId: string) => {
    ...
    const { data } = await supabase.from('email_campaign_contacts')
      .select('id, email, nome, status, error_message, sent_at')
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground">Gerencie seus disparos de e-mail em massa.</p>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-10 text-center">
            ...
            <p className="text-sm text-muted-foreground">Nenhuma campanha de e-mail criada.</p>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-[95vw] sm:max-w-lg max-h-[90vh] overflow-y-auto p-4 sm:p-6">
          <DialogHeader>
            <DialogTitle className="text-base">Nova Campanha de E-mail</DialogTitle>
            <DialogDescription className="text-xs">Configure os detalhes do disparo de e-mail.</DialogDescription>
          </DialogHeader>
          <div className="space-y-5">
            <div className="space-y-3">
              ...
              <div>
                ...
                <Input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex: Newsletter Janeiro" className="mt-1" />
              </div>
              <div>
                <Label className="text-xs">Assunto do E-mail *</Label>
                <Input value={assunto} onChange={(e) => setAssunto(e.target.value)} placeholder="Ex: Novidades para {nome}" className="mt-1" />
              ...
              <div>
                ...
                <Textarea value={mensagem} onChange={(e) => { setMensagem(e.target.value); setAiVariations([]); }} placeholder="Escreva o conteúdo do e-mail..." rows={5} className="mt-1 text-sm" />
                <p className="text-[10px] text-muted-foreground mt-1">Use {'{nome}'} para personalizar. Suporta HTML.</p>
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground">Gera versões únicas do e-mail para cada contato</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {aiVariations.length > 0 && (
                      <div className="space-y-1.5">
                        ...
                        <p className="text-[10px] text-muted-foreground italic">💡 Ao disparar, a IA gerará uma variação única para cada contato.</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="space-y-3">
                      ...
                      : filteredIcps.map((icp) => (
                        <label key={icp.id} className={`flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-colors ${selectedIcps.includes(icp.id) ? 'bg-primary/5 border border-primary/20' : 'hover:bg-muted/50 border border-transparent'}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-muted-foreground">{icp.emailCount || 0} leads com e-mail</p>
                          </div>
                        </label>
                      ))}
                  </div>
              ...
              {contactSource === 'csv' && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <Button size="sm" variant="outline" className="text-xs gap-1.5 h-8" onClick={downloadEmailCsvTemplate}><Download className="w-3.5 h-3.5" /> Baixar Modelo</Button>
                  ...
                  <div className="p-3 rounded-xl bg-muted/30 border border-border">
                    <p className="text-[10px] text-muted-foreground leading-relaxed">
                      ...
                      <strong>Colunas:</strong> <code className="bg-muted px-1 rounded">email</code> (obrig.), <code className="bg-muted px-1 rounded">nome</code> (opcional)
                    </p>
                  </div>
              ...
              {totalSelectedContacts > 0 && (
                <div className="flex items-center gap-2 p-2.5 rounded-xl bg-primary/5 border border-primary/20">
                  ...
                  <p className="text-xs text-primary font-medium">{totalSelectedContacts} contato{totalSelectedContacts > 1 ? 's' : ''} com e-mail</p>
                </div>
              )}
            </div>
          ...
          <div className="overflow-x-auto -mx-4 sm:mx-0">
            <Table>
              <TableHeader><TableRow>
                <TableHead className="text-xs">E-mail</TableHead><TableHead className="text-xs">Nome</TableHead>
                ...
                <div>
                  ...
                  <div className="max-h-48 overflow-y-auto border rounded-xl overflow-x-auto">
                    <Table><TableHeader><TableRow>
                      <TableHead className="text-xs">E-mail</TableHead><TableHead className="text-xs">Status</TableHead><TableHead className="text-xs">Intervalo</TableHead>
                    </TableRow></TableHeader>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-[95vw] sm:max-w-md p-4 sm:p-6">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">Altere o nome, assunto ou mensagem.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div>
            ...
            <div>
              ...
              <Textarea value={editMensagem} onChange={(e) => setEditMensagem(e.target.value)} rows={4} className="mt-1 text-sm" />
            </div>
